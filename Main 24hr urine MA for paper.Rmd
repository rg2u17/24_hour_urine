...
The race-specific estimates are underpowered and unstable – many are single studies or have implausibly wide ranges. On Wald tests: The only between group differences that were significant were black (1.40-1.75, n=304, I2=60%) vs white women (1.76-2.05, n=3260, I2=78%), and all races (0.94-2.40, n=4177, I2=99%) vs white women. In the meta-regression of women only, there is a significant decrease in the lower limit (from 0.94 in main MA to 0.55). The meta-regression for women (all races) seems robust with minimal heterogeneity and non significant Cochran’s Q, only 7/20 studies were included due to missing data (mainly standard deviations). This is in keeping with the observed instability of the race-specific sub-analyses. <br>
Therefore, we suggest using **age stratified ranges only**, without subdivision by sex or race.
<br>
...
volume_ma_2nd_analysis$main_meta_analysis$plot %+% 
  subset(volume_ma_2nd_analysis$main_meta_analysis$plot$data, 
         !age_band %in% c("Overall", "Meta-Regression"))
```
\newpage
...
volume_ma_2nd_analysis$ma_results %>% 
  filter(
    ...
    !age_band %in% c("Meta-Regression", "Overall")
  ) %>%
  mutate(
  ...
  tab_header(
    title = "Main Meta-analysis Results by Age Band",
...
volume_rob <- urine_ma2 %>%
  drop_na(volume_sd_m_l) %>%
  left_join(
    overall_bias, by = c("study_name" = "study")
  ) %>%
  select(study_name, age_band, overall, study_type) %>%
  mutate(
    overall = case_when(
...
volume_grade <- cbind(
  age_band = c("18+", "20-30", "30-40", "40-50", "50-60", "60+"),
  grade = c(grade_18plus$grade_rating,
            grade_20_30$grade_rating,
            grade_30_40$grade_rating,
            grade_40_50$grade_rating,
            grade_50_60$grade_rating,
            grade_60plus$grade_rating)
) %>% as_tibble() %>%
  mutate(
...
### 24 Hour Urine Volume Suggested Reference Range
```{r}
volume_ma_2nd_analysis$ma_results %>% 
  filter(
    sex_group == "main meta-analysis",
