...
sex_age_subdivision_ma <- function(urine_component,
  ...
  # RCT vs All studies comparison
  rct_comparisons <- purrr::map_dfr(sex_groups, function(sex) {
    all_studies <- combined_results %>%
      filter(sex_group == sex, as.character(age_band) == "Overall")
    rct_studies <- combined_results %>%
      filter(sex_group == sex, as.character(age_band) == "Overall (RCT only)")
  ...
  combined_summary_table <- combined_results %>%
    ...
    mutate(
      ...
      Studies = ifelse(is.na(n_studies), "—", as.character(n_studies)),
      "Study Type" = ifelse(is.na(study_type), "—", as.character(study_type)),
      Participants = ifelse(is.na(total_participants), "—", as.character(total_participants)),
      I2 = I2 * 100,
      `I²` = ifelse(is.na(I2), "—", paste0(round(I2, 1), "%")),
      `Cochran's Q` = ifelse(is.na(Q), "—", as.character(round(Q, 1))),
  ...
  combined_summary_gt <- combined_summary_table %>%
    ...
    # Highlight by sex group (keep original colors)
    tab_style(
      style = cell_fill(color = "springgreen3"),
      locations = cells_body(rows = `Sex Group` == "main meta-analysis")
    ) %>%
    ...
    # Bold the RCT only rows
    tab_style(
      style = cell_text(weight = "bold"),
      locations = cells_body(
        columns = everything(),
        rows = grepl("RCT only", `Age Band`)
      )
    ) %>%
    tab_options(
      table.font.size = px(11),
...
#### Summary Table
```{r}
phosphorus_ma_2nd_analysis$combined_summary_table
```
\newpage
...
volume_grade<- calculate_grade(
  ...
  risk_of_bias = "Serious", #from bias analysis
...
calcium_grade <- calculate_grade(
  ...
  risk_of_bias = "Moderate", # from bias analysis
...
ph_grade <- calculate_grade(
  ...
  risk_of_bias = "Serious",
  inconsistency = "Moderate", # Strongly consistent throughout studies
  indirectness = "Moderate",
...
## Phosphorous {.tabset .tabset-pills}
### Comment
High heterogeneity across all analyses, with significant amount of data missing for sub-analyses. Meta-regression reduces heterogeneity in overall and female only analyses, suggesting study-level covariates partially explain observed variation. Wald tests suggest significant differences between age groups. However, there seems to be insufficient data to rely on these age-stratified results. <br>
	We recommend sex-specific results may be used as a pooled population distribution/expected distribution across heterogenous studies. <br>


### Plot
```{r}
male_plot <- phosphorus_ma_2nd_analysis$male$plot %+%
  subset(phosphorus_ma_2nd_analysis$male$plot$data, age_band == "Overall") +
  scale_x_continuous(limits = c(0,20))

female_plot <- phosphorus_ma_2nd_analysis$female$plot %+%
  subset(phosphorus_ma_2nd_analysis$female$plot$data, age_band == "Overall") +
  scale_x_continuous(limits = c(0,20))

phosphorus_final_plot <- (male_plot / female_plot) + plot_layout(axes = "collect")

phosphorus_final_plot
```
\newpage

### Results Table
```{r}
phosphorus_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>% filter(
    sex_group != "main meta-analysis"
  ...
  tab_header(
    title = "phosphorus Meta-analysis Results by Age Band",
    subtitle = "Random-effects meta-analysis"
  ) %>%
  tab_source_note(
    source_note = md(
...
### Bias analysis
```{r}
phosphorus_rob <- urine_ma2 %>%
  drop_na(phosphorus_sd_mmol_24h) %>%
  left_join(
    overall_bias, by = c("study_name" = "study")
  ) %>%
  select(study_name, overall, study_type, male_female) %>%
  ...
  tab_header(
    title = "Risk of Bias by Study and Age Band for 24 Hour Urine phosphorus"
  )
...
### 24 Hour Urine Phosphorus Suggested Reference Range inc. GRADE
```{r}
phosphorus_male_grade <- calculate_grade(
  study_design= "Mixed",
  risk_of_bias = "Serious",
  inconsistency = "Serious",
  indirectness = "Moderate",
...
phosphorus_female_grade <- calculate_grade(
  study_design= "Mixed",
  risk_of_bias = "Moderate",
  inconsistency = "Serious",
  indirectness = "Moderate",
...
phosphorus_grade <- cbind(
  sex = c("Male", "Female"),
  grade = c(phosphorus_male_grade$grade_rating,
            phosphorus_female_grade$grade_rating)
) %>% as_tibble() %>%
  mutate(
    grade_rating = grade_symbol(grade)
  )

phosphorus_final_results <- phosphorus_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
phosphorus_final_results %>%
  ...
  tab_header(
    title = "Finalised Meta-Analysis Results for Phosphorus",
    subtitle = "Subdivided by Age Band"
  ) %>%
  tab_source_note(
    source_note = md(
...
phosphorus_final_results <- phosphorus_final_results %>% 
  mutate(
    urine_component = "Phosphorus"
  )
```
\newpage
...
## Sodium {.tabset .tabset-pills}
### Comment
There is substantial variability across all analyses. There appear to be differences in sex and age bands, although only age-band 30-40 demonstrates significant differences between sexes (all other wald tests are non-significant). There are reductions in heterogeneity in specific scenarios e.g. males aged 40-50. However, this is likely an artifact of small numbers of patients/studies <br>
  The 40–50 age band and the female 50–60 subgroup are the most promising candidates for clinically useful ranges. However, for other subgroups we suggest using the sex-stratified results for potential reference ranges. <br>


### Plot
```{r}
male_plot <- sodium_ma_2nd_analysis$male$plot %+%
  subset(sodium_ma_2nd_analysis$male$plot$data, age_band == "Overall") +
  scale_x_continuous(limits = c(90,230))

female_plot <- sodium_ma_2nd_analysis$female$plot %+%
  subset(sodium_ma_2nd_analysis$female$plot$data, age_band == "Overall") +
  scale_x_continuous(limits = c(90,230))

sodium_final_plot <- (male_plot / female_plot) + plot_layout(axes = "collect")
...
### 24 Hour Urine Sodium Suggested Reference Range inc. GRADE
```{r}
sodium_male_grade <- calculate_grade(
  study_design= "Mixed",
  risk_of_bias = "Serious",
  inconsistency = "Serious",
  indirectness = "Moderate",
...
sodium_female_grade <- calculate_grade(
  study_design= "Mixed",
  risk_of_bias = "Moderate",
  inconsistency = "Serious",
  indirectness = "Moderate",
...
sodium_grade <- cbind(
  sex = c("Male", "Female"),
  grade = c(sodium_male_grade$grade_rating,
            sodium_female_grade$grade_rating)
) %>% as_tibble() %>%
  mutate(
    grade_rating = grade_symbol(grade)
  )

sodium_final_results <- sodium_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
### 24 Hour Urine Potassium Suggested Reference Range inc. GRADE
```{r}
...
potassium_final_results <- potassium_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
High heterogeneity throughout analyses, with substantial, albeit non-significant, variation between strata. No clear consistent fluctuation between age groups. Meta-regression slightly reduces the intervals and I2, but substantial heterogeneity remains suggesting study-level covariates only partially explaining variability. <br>
  Meta-analysis of RCTs demonstrates similar heterogeneity, however the results are broadly in line with the remainder of the analyses. <br>
	We recommend the overall meta-regression may be used as a pooled population distribution/expected distribution across heterogenous studies. <br>
...
### 24 Hour Urine Magnesium Suggested Reference Range inc. GRADE
```{r}
magnesium_grade <- calculate_grade(
  study_design= "RCT", #RCTs have similar ranges to MR result
  risk_of_bias = "Moderate", #consistent results
  inconsistency = "Moderate",
  indirectness = "Moderate",
  imprecision = "Moderate",
...
magnesium_final_results <- magnesium_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
Heterogeneity remains high across analyses, however there is reasonable consistency across strata. Meta-regression reduces both interval and I2, suggesting at least partial adjustment for between study variation. Few studies in men alone and no meta-regression in this group, and therefore the 30-40 vs 60+ significant wald test result is not interpretable. Race sub-analyses are underpowered. <br>
	We recommend the overall meta-regression may be used as a pooled population distribution/expected distribution across heterogenous studies. The female only meta-regression could also be used, however there is no male equivalent. <br>
...
### 24 Hour Urine ammonium Suggested Reference Range inc. GRADE
```{r}
...
ammonium_final_results <- ammonium_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
Substantial heterogeneity throughout analyses. Minimal data for age and race stratifications. Meta-regression does reduce interval and I2 in both male and female analyses. <br>
	We recommend using the overall meta-regression result given the poor amount of evidence in other strata. However, this should be interpreted as a pooled population distribution.
 <br>
...
### 24 Hour Urine chloride Suggested Reference Range inc. GRADE
```{r}
...
chloride_final_results <- chloride_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
There are few studies in this analysis, with wide variation in results and high heterogeneity throughout the different strata. The meta-regression result is not valid. There is marked variation between men and women, although non-significant and appear due to variation within the substantially larger female cohort.<br>
	The overall meta-analysis result has the most robust evidence given the similarity to RCT results. <br>
...
### 24 Hour Urine sulphate Suggested Reference Range inc. GRADE
```{r}
sulphate_grade <- calculate_grade(
  study_design= "RCT", #Consistent with RCT results
  risk_of_bias = "Strongly Suspected", 
  inconsistency = "Strongly Suspected",
  indirectness = "Moderate",
  imprecision = "Moderate",
  publication_bias = "Strongly suspected", #significant Q
  large_effect = TRUE,
...
sulphate_final_results <- sulphate_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Overall")
  ) %>%
  filter(
...
### 24 Hour Urine urea Suggested Reference Range inc. GRADE
```{r}
urea_grade <- calculate_grade(
  study_design= "Mixed", #Consistent with RCT results
  risk_of_bias = "Strongly Suspected", 
  inconsistency = "Strongly Suspected",
  indirectness = "Moderate",
  imprecision = "Moderate",
...
urea_final_results <- urea_ma_2nd_analysis$ma_results %>% 
  filter(
    age_band %in% c("Meta-Regression")
  ) %>%
  filter(
...
## All MA Results
```{r}
# All the other phosphate reference ranges seem to be aligned to the phosphorous reference range not the phosphate
phosphate_final_results <- phosphorus_final_results %>%
  mutate(
    urine_component = "phosphate"
  )
...
final_gt <- final_table %>%
  ...
  tab_header(
    title = "Finalised 24 Hour Urine Reference Range Suggestions",
...
gt_tbl <- summary_table_separated %>%
  ...
  left_join(
    proposed_lookup %>% select(join_key, sex_group, Proposed, `Proposed Rating`),
  ...
  cols_align("center",
             c(EAU, NDH, NHS, Litholink, `Mayo Clinic`, Proposed, `Proposed Rating`)) %>%
